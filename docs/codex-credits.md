...
发送成功后，后端以脱离请求取消的 8 秒上下文刷新次数，再使用独立的 3 秒上下文保存结果。回读耗尽期限仍可清空过期快照，并返回 `sent: true`、`refresh_failed: true`。两个展示快照字段不触发调度事件或调度桶重建。

发送请求不自动重试。公共 Axios 拦截器保留网络错误码；发送时断网、超时或取消均提示结果不确定，要求先在 Codex 核对邀请状态，并禁用再次发送，直到手动刷新资格。只读查询失败显示查询错误。接口错误不会向前端透传上游原始响应中的凭证或无关个人信息。

邀请业务通过 `service.OpenAIReferralClient` 接口调用仓储层 HTTP 适配器。Service 负责账号、计划、次数和同意校验；适配器负责代理客户端、请求协议、响应解析、错误脱敏与禁止重试，通过 Wire 注入。

### 自动回归检查

`make test-frontend` 的 CI 测试清单包含邀请组件、真实 Axios 拦截器与邀请组件联动测试、点数组件测试。后端单元测试覆盖回读期限耗尽后清理缓存、展示快照不写调度 outbox，以及邀请适配器协议、错误脱敏和断连不重试。
