...
func (_q *APIKeyQuery) QueryExternalUserMappings() *ExternalUserMappingQuery {
...
func (_q *APIKeyQuery) WithExternalUserMappings(opts ...func(*ExternalUserMappingQuery)) *APIKeyQuery {
	query := (&ExternalUserMappingClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *APIKeyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*APIKey, error) {
	...
	if query := _q.withExternalUserMappings; query != nil {
		if err := _q.loadExternalUserMappings(ctx, query, nodes,
			func(n *APIKey) { n.Edges.ExternalUserMappings = []*ExternalUserMapping{} },
			func(n *APIKey, e *ExternalUserMapping) {
...
func (_q *APIKeyQuery) loadExternalUserMappings(ctx context.Context, query *ExternalUserMappingQuery, nodes []*APIKey, init func(*APIKey), assign func(*APIKey, *ExternalUserMapping)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(externalusermapping.FieldAPIKeyID)
	}
	query.Where(predicate.ExternalUserMapping(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(apikey.ExternalUserMappingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
