...
// is followed by requested_reasoning_effort and created_at.
func TestPrepareUsageLogInsert_SessionIDArgWiring(t *testing.T) {
	require.Len(t, usageLogInsertArgTypes, 60, "arg-type table must include session_id and merged usage audit fields")
...
func TestPrepareUsageLogInsert_RequestedReasoningEffortWiring(t *testing.T) {
	requested := "high"
	log := newSessionIDUsageLog(nil)
	log.RequestedReasoningEffort = &requested

	arg := prepareUsageLogInsert(log).args[len(usageLogInsertArgTypes)-2]
