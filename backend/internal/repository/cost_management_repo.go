...
func (r *costManagementRepository) GetCostOverview(ctx context.Context, start, end time.Time) (*service.CostOverview, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT COALESCE(SUM(amount_cny) FILTER(WHERE aggregate_scope='usage' AND calculation_status='calculated'),0)::text,COALESCE(SUM(amount_cny) FILTER(WHERE aggregate_scope='fixed_plan_total' AND calculation_status='calculated'),0)::text,COALESCE(SUM(amount_cny) FILTER(WHERE aggregate_scope IN('usage','fixed_plan_total') AND calculation_status='calculated'),0)::text,COALESCE(SUM(pending_count) FILTER(WHERE calculation_status='pending' AND EXISTS(SELECT 1 FROM accounts a WHERE a.id=cost_daily_aggregates.account_id AND a.deleted_at IS NULL)),0),COALESCE(SUM(error_count),0),COALESCE(SUM(request_count) FILTER(WHERE aggregate_scope='usage'),0),COALESCE(SUM(request_count) FILTER(WHERE aggregate_scope='usage' AND calculation_status='calculated'),0) FROM cost_daily_aggregates WHERE bucket_date >= $1::date AND bucket_date < $2::date`, start, end).Scan(&o.DynamicCostCNY, &o.FixedCostCNY, &o.TotalCostCNY, &o.PendingCount, &o.ErrorCount, &o.EligibleCount, &o.CalculatedCount)
	if err != nil {
		return nil, err
	}
