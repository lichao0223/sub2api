...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		// 系统设置
		registerSettingsRoutes(admin, h, loginIPBlocker)
...
func registerWorkInsightRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	insights := admin.Group("/ai-work-insights")
	{
		...
		insights.POST("/analyze-now", h.Admin.WorkInsight.AnalyzeNow)
		...
		insights.DELETE("/batches/errors", h.Admin.WorkInsight.DeleteAllFailedBatches)
...
func registerCostManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	costs := admin.Group("/cost-management")
	{
		...
		costs.POST("/subscription-units", h.Admin.CostManagement.CreateSubscriptionUnit)
		costs.PUT("/subscription-units/:id", h.Admin.CostManagement.RenameSubscriptionUnit)
		costs.POST("/subscription-units/:id/end", h.Admin.CostManagement.EndSubscriptionUnit)
		...
		costs.POST("/plans/:id/price-changes", h.Admin.CostManagement.ChangePlanPrice)
		costs.GET("/plans/:id/price-history", h.Admin.CostManagement.ListPriceHistory)
...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.POST("/:id/api-keys/rotate", h.Admin.APIKey.RotateUserKeys)
...
func registerGroupRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	groups := admin.Group("/groups")
	{
		...
		groups.GET("/:id/available-models", h.Admin.Group.GetAvailableModels)
...
func registerSettingsRoutes(admin *gin.RouterGroup, h *handler.Handlers, loginIPBlocker *appmiddleware.LoginIPBlocker) {
	adminSettings := admin.Group("/settings")
	{
		adminSettings.GET("", h.Admin.Setting.GetSettings)
		adminSettings.PUT("", h.Admin.Setting.UpdateSettings)
		registerLoginIPBlockRoutes(adminSettings, loginIPBlocker)
...
func registerUsageRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	usage := admin.Group("/usage")
	{
		...
		usage.POST("/external-imports/preview", h.Admin.Usage.PreviewExternalUsageImport)
		usage.POST("/external-imports", h.Admin.Usage.ImportExternalUsage)
		usage.GET("/external-imports", h.Admin.Usage.ListExternalUsageImportBatches)
