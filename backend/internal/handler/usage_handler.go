...
func parseUserTimeRange(c *gin.Context) (time.Time, time.Time) {
	return parseUserTimeRangeValues(c.Query("start_date"), c.Query("end_date"), c.Query("timezone"))
}
...
// DashboardTokenRanking handles getting cross-user token usage ranking.
// GET /api/v1/usage/dashboard/token-ranking
func (h *UsageHandler) DashboardTokenRanking(c *gin.Context) {
...
func (h *UsageHandler) DashboardNonworkTokenRanking(c *gin.Context) {
	...
	ranking, err := h.usageService.GetUserNonworkTokenRanking(c.Request.Context(), startTime, endTime.Add(-24*time.Hour), scope, rankBy, sortOrder, userTZ, req.ExternalOrganizationIDs, strings.TrimSpace(req.Username), limit)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
