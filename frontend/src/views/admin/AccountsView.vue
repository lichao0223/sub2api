        ...
        >
          ...
          <template #cell-schedulable="{ row }">
            <div class="flex items-center gap-2">
              ...
              <span v-if="row.schedule_status === 'outside_window'" class="text-xs text-gray-500" :title="row.schedule_timezone ? `下一次开始：${row.next_schedule_start || '未配置'}` : '非调度时段'">非调度时段</span>
            </div>
          </template>
