  ...
  <AppLayout>
    ...
    <BaseDialog :show="breakdownDialog" :title="breakdownTitle" width="extra-wide" @close="breakdownDialog=false">
      <div class="space-y-3">
        ...
        <div class="max-h-[60vh] overflow-auto"><table class="table"><thead class="sticky top-0 bg-white dark:bg-dark-900"><tr><th>类型</th><th>成本方案</th><th>账号/订阅实例</th><th>上游模型</th><th>计价依据</th><th>请求数</th><th>输入</th><th>缓存写入</th><th>缓存读取</th><th>输出</th><th>金额</th></tr></thead><tbody><tr v-for="(item,index) in costBreakdown.items" :key="index"><td>{{ item.cost_mode==='metered'?'按量':'固定' }}</td><td>{{ item.plan_name||'未匹配方案' }}</td><td>{{ item.account_name||item.subscription_unit_name||'-' }}</td><td>{{ item.upstream_model||'-' }}</td><td class="min-w-64 text-xs">{{ pricingText(item) }}</td><td>{{ item.request_count||'-' }}</td><td>{{ item.input_tokens||'-' }}</td><td>{{ item.cache_write_tokens||'-' }}</td><td>{{ item.cache_read_tokens||'-' }}</td><td>{{ item.output_tokens||'-' }}</td><td class="font-medium">{{ money(item.amount_cny) }}</td></tr></tbody></table><div v-if="!costBreakdown.items.length" class="py-8 text-center text-sm text-gray-400">当前范围暂无已核算成本</div></div>
      </div>
...
function recalculateCurrentRange(){const end=range.end<yesterday?range.end:yesterday;if(range.start>end){app.showError('当天数据由增量任务核算，历史补算最晚只能选择昨天');return}Object.assign(recalc,{start_date:range.start,end_date:end});pendingDialog.value=false;submitRecalc()}
