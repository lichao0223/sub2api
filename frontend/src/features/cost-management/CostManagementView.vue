  ...
  <AppLayout>
    ...
    <BaseDialog :show="accountDialog" :title="batchMode?'批量配置账号成本':'配置账号成本'" @close="accountDialog=false">
      <div class="space-y-4">
        ...
        <div v-if="accountForm.cost_mode==='fixed'"><label class="input-label">归属订阅实例</label><Select v-model="accountForm.subscription_unit_id" :options="subscriptionUnitOptions" /><p class="mt-1 text-xs text-gray-500">同一份订阅建立多个平台账号时，请选择同一个实例，只计算一份成本。</p></div>
