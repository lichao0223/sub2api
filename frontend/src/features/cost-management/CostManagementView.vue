...
async function loadAnalysis(period=chartPeriod.value){chartPeriod.value=period;Object.assign(analysis,await costManagementAPI.analysis(period));await nextTick();chart.value?.destroy();if(!chartCanvas.value)return;chart.value=new Chart(chartCanvas.value,{type:['month','year'].includes(period)?'bar':'line',data:{labels:analysis.trend.map(x=>x.bucket),datasets:[{label:'总成本',data:analysis.trend.map(x=>+x.total_cost_cny),borderColor:'#7a5af8'},{label:'按量成本',data:analysis.trend.map(x=>+x.dynamic_cost_cny),borderColor:'#0866ed'},{label:'固定成本',data:analysis.trend.map(x=>+x.fixed_cost_cny),borderColor:'#ff7800'}]},options:{responsive:true,maintainAspectRatio:false}})}
...
async function editPlan(x:CostPlan){const p=await costManagementAPI.plan(x.id);planDialogMode.value='edit';editingPlanId.value=x.id;pricingPlan.value=undefined;Object.assign(planForm,{...p,prices:p.prices?.length?p.prices.map(y=>({...y})):[emptyPrice()]});planDialog.value=true}
async function openPriceChange(x:CostPlan,unitIDs?:number[]){const p=await costManagementAPI.plan(x.id);planDialogMode.value='price';pricingPlan.value=p;editingPlanId.value=undefined;const earliest=Math.max(Date.now(),new Date(p.effective_from).getTime()+60000);Object.assign(planForm,{...p,effective_from:dateTimeLocal(new Date(earliest)),prices:p.prices?.length?p.prices.map(y=>({...y})):[emptyPrice()]});priceTargetUnits.value=p.plan_type==='fixed'?(await costManagementAPI.subscriptionUnits(p.id)).filter(unit=>!unit.effective_to):[];priceSubscriptionUnitIDs.value=unitIDs||priceTargetUnits.value.map(unit=>unit.id);priceUpdateDefault.value=!unitIDs;planDialog.value=true}
