<template>
  <AppLayout>
    ...
    <BaseDialog :show="subscriptionManagerDialog" :title="`订阅实例 · ${managingPlan?.name||''}`" width="extra-wide" @close="closeSubscriptionManager">
      <div class="space-y-5">
        <div v-if="managingPlan?.status==='active'" class="flex items-end gap-3 rounded-lg bg-gray-50 p-4 dark:bg-dark-800">
          <div class="flex-1"><label class="input-label">实例名称</label><input v-model="subscriptionUnitName" class="input w-full" placeholder="例如：ChatGPT Plus #3" @keyup.enter="saveSubscriptionUnit"></div>
        ...
        <div class="overflow-x-auto"><table class="table"><thead><tr><th>实例名称</th><th>绑定账号</th><th>创建时间</th><th>停用时间</th><th>状态</th><th>操作</th></tr></thead><tbody><tr v-for="unit in managedSubscriptionUnits" :key="unit.id"><td>{{ unit.name }}</td><td>{{ unit.account_count }}</td><td class="whitespace-nowrap">{{ dateTime(unit.created_at) }}</td><td class="whitespace-nowrap">{{ dateTime(unit.ended_at) }}</td><td>{{ unit.ended_at?'已停用':'使用中' }}</td><td class="space-x-3"><button v-if="!unit.ended_at&&managingPlan?.status==='active'" class="text-primary-600" @click="editSubscriptionUnit(unit)">修改名称</button><button v-if="!unit.ended_at&&managingPlan?.status==='active'" class="text-red-500" @click="endSubscriptionUnitTarget=unit">停用</button><span v-if="unit.ended_at||managingPlan?.status!=='active'" class="text-gray-400">-</span></td></tr></tbody></table><div v-if="!managedSubscriptionUnits.length" class="py-8 text-center text-sm text-gray-400">暂无订阅实例</div></div>
      </div>
      <template #footer><button class="btn btn-secondary" @click="closeSubscriptionManager">关闭</button></template>
    </BaseDialog>
    <ConfirmDialog :show="!!endSubscriptionUnitTarget" title="停用订阅实例" :message="`确认停用“${endSubscriptionUnitTarget?.name||''}”？当前绑定账号的成本归属也会同时结束。`" confirm-text="确认停用" :loading="subscriptionUnitSubmitting" danger @confirm="confirmEndSubscriptionUnit" @cancel="endSubscriptionUnitTarget=undefined" />
...
const subscriptionUnitOptions=computed(()=>[
  ...subscriptionUnits.value.filter(x=>!x.ended_at).map(x=>({value:x.id,label:`${x.name}（${x.account_count} 个账号）`})),
...
const subscriptionManagerDialog=ref(false),managingPlan=ref<CostPlan>(),managedSubscriptionUnits=ref<CostSubscriptionUnit[]>([]),subscriptionUnitName=ref(''),editingSubscriptionUnitId=ref<number>(),subscriptionUnitSubmitting=ref(false),endSubscriptionUnitTarget=ref<CostSubscriptionUnit>()
...
function editSubscriptionUnit(unit:CostSubscriptionUnit){editingSubscriptionUnitId.value=unit.id;subscriptionUnitName.value=unit.name}
function cancelEditSubscriptionUnit(){editingSubscriptionUnitId.value=undefined;subscriptionUnitName.value=''}
...
async function saveSubscriptionUnit(){if(!managingPlan.value||subscriptionUnitSubmitting.value)return;subscriptionUnitSubmitting.value=true;try{editingSubscriptionUnitId.value?await costManagementAPI.renameSubscriptionUnit(editingSubscriptionUnitId.value,subscriptionUnitName.value):await costManagementAPI.createSubscriptionUnit(managingPlan.value.id,subscriptionUnitName.value);app.showSuccess(editingSubscriptionUnitId.value?'订阅实例名称已修改':'订阅实例已创建');cancelEditSubscriptionUnit();await refreshSubscriptionUnits()}catch(e:any){app.showError(e.message||'保存订阅实例失败')}finally{subscriptionUnitSubmitting.value=false}}
async function confirmEndSubscriptionUnit(){if(!endSubscriptionUnitTarget.value||subscriptionUnitSubmitting.value)return;subscriptionUnitSubmitting.value=true;try{await costManagementAPI.endSubscriptionUnit(endSubscriptionUnitTarget.value.id);app.showSuccess('订阅实例已停用');endSubscriptionUnitTarget.value=undefined;await refreshSubscriptionUnits();await loadAccounts()}catch(e:any){app.showError(e.message||'停用订阅实例失败')}finally{subscriptionUnitSubmitting.value=false}}
