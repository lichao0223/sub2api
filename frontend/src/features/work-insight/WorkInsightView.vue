<template>
  <AppLayout>
    <div class="mx-auto max-w-[1600px] pb-24">
      ...
      <BaseDialog :show="usageDetailOpen" :title="usageDetailUser ? `${usageDetailUser.username || `用户 ${usageDetailUser.user_id}`} 的预警使用记录` : '预警使用记录'" width="extra-wide" close-on-click-outside @close="usageDetailOpen = false">
        ...
        <div class="space-y-5">
          ...
          <ConfigCard title="使用预警" description="从使用记录中找出单次输入 Token 超过阈值的用户，历史记录无需重建">
            ...
            <div v-if="draft.usage_alert_enabled" class="mt-5 border-t border-gray-200 pt-5 dark:border-dark-700">
              <label class="flex items-center justify-between gap-4"><span><strong>自动禁用超限用户</strong><small>当天超过阈值的请求达到次数后，直接禁用用户，不自动恢复</small></span><Toggle v-model="draft.usage_alert_auto_disable_enabled" aria-label="自动禁用超限用户" /></label>
              <div v-if="draft.usage_alert_auto_disable_enabled" class="mt-4 grid gap-4 sm:grid-cols-2">
                <NumberField v-model="draft.usage_alert_consecutive_count" label="当天超限次数" suffix="次" :min="1" :max="100" />
                <label class="field sm:col-span-2"><span>用户白名单</span><OpenAIFastPolicyUserSelector v-model="draft.usage_alert_exempt_user_ids" /><small>白名单用户和管理员不会被自动禁用</small></label>
              </div>
            </div>
          </ConfigCard>
    ...
    <BaseDialog :show="detailOpen" :title="detail ? `${detail.insight.username} 的 AI 工作洞察` : 'AI 工作洞察'" width="wide" close-on-click-outside @close="detailOpen = false">
      <div v-if="detail" class="space-y-5">
        ...
        <section><h3 class="section-title">开发工具</h3><div class="mt-2 flex flex-wrap gap-2"><span v-for="tool in detail.developer_tools" :key="tool.name" class="badge">{{ tool.name }} · {{ formatNumber(tool.requests) }} 次 · 最近 {{ formatDateTime(tool.last_seen) }}</span><span v-if="!detail.developer_tools.length" class="text-sm text-gray-400">暂无可识别的开发工具</span></div><p class="mt-2 text-xs text-gray-400">根据请求 User-Agent 识别，工具名称仅作客户端标识参考。</p></section>
      ...
      <div class="space-y-6">
        ...
        <section class="overflow-hidden rounded-xl border border-gray-200 dark:border-dark-700">
          ...
          <div v-show="batchLogTab === 'errors'" role="tabpanel">
            ...
            <div class="overflow-x-auto"><table class="w-full text-left text-xs"><thead><tr class="border-b border-red-100 dark:border-red-900/50"><th class="p-3">时间</th><th class="p-3">用户</th><th class="p-3">样本</th><th class="p-3">错误详情</th><th class="p-3">状态</th><th class="p-3 text-right">操作</th></tr></thead><tbody>
            <tr v-for="item in errorLogPage.items" :key="item.id" class="border-b border-red-100 last:border-0 dark:border-red-950"><td class="p-3 whitespace-nowrap">{{ formatDateTime(item.created_at) }}</td><td class="p-3">{{ logUserLabel(item.username, item.user_id) }}</td><td class="p-3">{{ item.sample_count }}</td><td class="max-w-md p-3 text-red-600 dark:text-red-400"><div>{{ errorLabel(item.error_code) }}</div><code class="mt-1 block text-[10px] text-red-400">{{ item.error_code || 'unknown_error' }}</code><details v-if="item.error_detail" class="mt-2 text-gray-700 dark:text-gray-300"><summary class="cursor-pointer select-none text-xs font-medium">查看模型返回</summary><pre class="mt-2 max-h-48 overflow-auto whitespace-pre-wrap break-words rounded bg-gray-100 p-2 text-[11px] dark:bg-dark-800">{{ item.error_detail }}</pre></details></td><td class="p-3">{{ statusLabel(item.status) }}</td><td class="p-3"><div class="flex justify-end gap-2"><button type="button" class="btn btn-primary btn-xs" :disabled="retryingBatchID !== null || retryingAll || deletingBatchID !== null || deletingAll" @click="retryBatch(item)">{{ retryingBatchID === item.id ? '重新排队中…' : '重新分析' }}</button><button v-if="item.status === 'retry'" type="button" class="btn btn-secondary btn-xs" :disabled="stoppingBatchID !== null || deletingBatchID !== null || deletingAll" @click="stopBatch(item)">{{ stoppingBatchID === item.id ? '暂停中…' : '暂停重试' }}</button><button type="button" class="btn btn-danger btn-xs" :disabled="deletingBatchID !== null || deletingAll" @click="deleteBatch(item)">{{ deletingBatchID === item.id ? '删除中…' : '删除' }}</button></div></td></tr>
...
const usageDetailPage = reactive<LogPage<AdminUsageLog>>({ items: [], total: 0, page: 1, page_size: 20, pages: 0 })
...
function clientLabel(userAgent: string | null): string {
  ...
  for (const [name, keys] of clients) if (keys.some(key => ua.includes(key))) return name
  return ua ? '其他客户端' : '未知客户端'
}
...
async function loadRankingSettings() { const settings = await adminAPI.settings.getSettings(); usdToCnyRate.value = Number(settings.token_ranking_usd_to_cny_rate) > 0 ? Number(settings.token_ranking_usd_to_cny_rate) : 7.2 }
...
async function loadUsageDetails() {
  ...
  try {
    const result = await adminUsageAPI.list({ user_id: usageDetailUser.value.user_id, start_date: alertStartDate.value, end_date: alertEndDate.value, min_input_tokens: draft.value?.usage_alert_input_tokens ?? 100000, page: usageDetailPage.page, page_size: usageDetailPage.page_size, exact_total: true, sort_by: 'created_at', sort_order: 'desc' })
    Object.assign(usageDetailPage, result)
    usageDetailLogs.value = result.items
  } catch { usageDetailLogs.value = []; showMessage('使用记录加载失败', true) } finally { usageDetailLoading.value = false }
}
async function changeUsageDetailPage(value: number) { usageDetailPage.page = value; await loadUsageDetails() }
async function changeUsageDetailPageSize(value: number) { usageDetailPage.page_size = value; usageDetailPage.page = 1; await loadUsageDetails() }
async function refresh() {
  loading.value = true
  try { const [, , , , accounts] = await Promise.all([loadConfig(), loadRuntime(), loadDaily(), loadOverview(), api.listAnalyzerAccounts(), loadAlerts(), loadRankingSettings()]); analyzerAccounts.value = accounts; message.value = '' }
