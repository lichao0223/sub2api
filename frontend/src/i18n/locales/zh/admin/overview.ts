export default {
    ...
    users: {
      ...
      apiKeyManagement: {
        ...
        group: '分组',
        selectGroup: '请选择分组',
        selectGroupHint: '选择一个分组后管理该分组下的 API Key。',
        user: '用户',
        name: '名称',
        selected: '已选择 {count} 个 API Key',
        selectedAll: '已选择该分组全部 {count} 个 API Key',
        ...
        apply: '批量设置',
        applying: '设置中...',
        confirm: '确定要更新 {count} 个 API Key 吗？',
        success: '已更新 {count} 个 API Key',
        moveAndRecreateSuccess: '已迁移 {count} 个并在原分组新建 {created} 个 API Key',
        failed: '批量更新 API Key 失败',
        deleteUngrouped: '删除全系统无分组 Key',
        ungroupedPreviewTitle: '确认删除 {count} 个无分组 API Key',
        ungroupedPreviewHint: '以下为全系统所有用户中未绑定有效分组的 API Key，包括未分配分组和原分组已删除的 Key。请确认后再删除。',
        deletedGroup: '原分组 #{id} 已删除',
        confirmDeleteUngroupedCount: '删除全部 {count} 个',
      ...
      rotateApiKeys: '轮换全部密钥',
      rotatingApiKeys: '轮换中...',
      rotateApiKeysConfirm: '确定轮换该用户的全部 API 密钥吗？旧密钥将立即失效。',
      rotateApiKeysSuccess: '全部 API 密钥已轮换',
      rotateApiKeysFailed: '轮换 API 密钥失败',
      group: '分组',
      none: '无',
